		...
		return [
			...
			[
				...
				'activate_callback' => function () {
					...
					$is_popup_enabled                      = Mods::get( self::MODS_COMPARSION_TABLE_DISPLAY_TYPE, 'page' ) === 'popup';
					$is_the_page_contains_comparison_table = is_a( $post, 'WP_Post' ) && ( Options::get_comparison_table_page_id() === $post->ID );
					$has_comparison_block                  = is_a( $post, 'WP_Post' ) && has_block( 'themeisle-blocks/woo-comparison', $post );

					// TODO: review the condition in the next versions. ( to avoid return true if popup enabled only. )
					return ( $is_popup_enabled || $is_the_page_contains_comparison_table || $has_comparison_block );
				},
			],
			[
