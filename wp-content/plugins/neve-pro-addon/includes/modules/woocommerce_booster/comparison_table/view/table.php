...
class Table {
	...
	public function add_comparison_table_class_to_body_classes( $classes ) {
		if ( $this->is_the_page_contains_comparison_table() || Options::current_page_related_with_wc() ) {
	...
	public function is_the_page_contains_comparison_table() {
		...
		return is_a( $post, 'WP_Post' ) && ( Options::get_comparison_table_page_id() === $post->ID );
	}
